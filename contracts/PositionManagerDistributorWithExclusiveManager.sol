...
/**
 * @title PositionManagerDistributorWithExclusiveManager
 ...
 */
contract PositionManagerDistributorWithExclusiveManager is IPositionManagerDistributor, IPancakeV3SwapCallback {
    ...
    /// @dev Exclusive manager address
    address private _exclusiveManager;

    /// @dev Percentage of fees for the exclusive manager with 4 decimals
    uint256 private _exclusiveManagerFeePercentage;
     ...
     * @param exclusiveManager Address of the exclusive manager (Could be address(0) if not needed)
     * @param exclusiveManagerFeePercentage Percentage of fees for the exclusive manager with 4 decimals
     */
    constructor(CreatePositionManagerParams memory params, address _pool, address exclusiveManager, uint256 exclusiveManagerFeePercentage) {
        if (_pool == address(0) || exclusiveManagerFeePercentage > MAX_PERCENTAGE) revert InvalidEntry();
        ...
        if (exclusiveManager != address(0) && exclusiveManagerFeePercentage > 0) {
            _exclusiveManager = exclusiveManager;
            _exclusiveManagerFeePercentage = exclusiveManagerFeePercentage;
        }
    ...
    function distributeRewards(address receiverAddress, uint256 receiverPercentage, uint256 amountOutMin) external {
        ...
        if (_exclusiveManager != address(0)) {
            uint256 exclusiveManagerAmount = FullMath.mulDiv(amountToDistribute, _exclusiveManagerFeePercentage, MAX_PERCENTAGE);

            usdt.safeTransfer(_exclusiveManager, exclusiveManagerAmount);

            amountToDistribute -= exclusiveManagerAmount;
        }
    ...
    /**
     * @notice Function to set the exclusive manager address and fee percentage
     * @param newExclusiveManager Address of the exclusive manager (Could be address(0))
     * @param newExclusiveManagerFeePercentage Percentage of fees for the exclusive manager with 4 decimals
     */
    function setExclusiveManagerData(address newExclusiveManager, uint256 newExclusiveManagerFeePercentage) external {
        if (newExclusiveManagerFeePercentage > MAX_PERCENTAGE) revert InvalidEntry();
